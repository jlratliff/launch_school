...
# puts <object>returns the object
# .class returns the class
# proc#call yields to the block without passing a parameter
# proc#call with an argument yields to the block passing the argument
...
# lambda has two forms of initialization in lines 17, 18
# puts <object> returns the object
# .class returns the class
# lambda#call with an argument yield to the block passing the argument
# lambda#call without a required argument prodduces an ArgumentError
...
# Yielding to a block is possible
# If no block is given in the function call a LocalJumpError is raised
...
# Missing arguments are ignored when yielding
# A block parameter ( |animal| in ine 53) must be present to receive a passed argument.
